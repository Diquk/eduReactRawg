...
export const TitleInfo = ({name, released, metacritic, genres, platforms, background_image}: GameItem) => {
  ...
  return (
    <div className="title-info" style={{backgroundImage: `url(${background_image})`}}>
      <h1 className="title-info__title">{name}</h1>
      {released && <p className="title-info__release">
        Release: {released} 
        {metacritic && <span className="rating rating_title title-info__rating">{metacritic}</span>}
      </p>}
